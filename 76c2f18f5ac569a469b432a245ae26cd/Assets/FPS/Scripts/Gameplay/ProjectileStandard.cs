...
namespace Unity.FPS.Gameplay
{
    public class ProjectileStandard : ProjectileBase
    {
        ...
        bool IsHitValid(RaycastHit hit)
        {
            ...
            if (hit.collider.tag == "NotIce" || hit.collider.tag == "Water")
            {
                return true;
            }
            else
        ...
        void OnHit(Vector3 point, Vector3 normal, Collider collider)
        {
            ...
            //make ice
            collider.tag = "Ice";
            collider.gameObject.transform.GetChild(0).GetComponent<Collider>().isTrigger = false;
            collider.gameObject.GetComponent<MeshRenderer>().material = iceMat;
